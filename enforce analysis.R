...
enforce.data <- read.csv("enforce data.csv")
attach(enforce.data)
...
plot2<-autoplot(p2, xlab="Distance to market (minutes)", ylab="Partial dependence", aes(size=1))+ylim(0, 0.8)+ 
  theme(legend.position="none") +theme(axis.text = element_text(size=5))+ theme(axis.title=element_text(size=7))+
  theme(axis.title.y = element_text(colour="white"))+ theme(axis.text.y = element_text(colour="white"))+ theme(axis.ticks.y = element_line(colour="white")) +
...
plot3<-autoplot(p3, xlab="Group size (households)", ylab="Partial dependence", aes(size=1))+ylim(0, 0.8) +  
  ...
  theme(panel.background = element_blank())+ theme(panel.border = element_rect(fill=NA) )+
  theme(axis.title.y = element_text(colour="white"))+ theme(axis.ticks.y = element_line(colour="white"))+theme(axis.text.y = element_text(colour="white"))+
...
pdf("fig3.pdf", width = 7, height = 3.5)
grid.arrange(plot1, plot2, plot3, ncol = 3, nrow=1, widths=c(2,2,2) )
dev.off()

tiff("figs2.tiff", units="in", res=300, width = 7, height = 10.5)
...
pdpplot3<- ggplot(data=pdp3plot, aes(x=uhhnum,y=yhat, group=Leader)) +  
  ...
  theme(axis.title.y = element_text(colour="white"))+theme(axis.text.y = element_text(colour="white"))+ggtitle("(B)") +  
  theme(legend.key = element_rect(fill = NA)) + theme(axis.ticks.y = element_line(colour="white"))+
  theme(plot.title=element_text(size=7, hjust=0.5))+ theme(legend.text=element_text(size=5))+
  theme(legend.title = element_text(size=7))
  

legend<-get_legend(pdpplot3)
pdpplot3<-pdpplot3 +theme(legend.position="none")


pdf("fig4.pdf",width =7, height=3.5)
grid.arrange(pdpplot2, pdpplot3, legend, ncol = 3, nrow=1, widths=c(3,3,1))
dev.off()

tiff("figs3.tiff", res=300, width = 4, height = 4)
pdpplot1
dev.off()
